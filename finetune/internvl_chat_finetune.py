...
class LazySupervisedDataset(Dataset):
    ...
    def video_get_item(self, data_item):
        ...
        special_tokens = '\n'.join(['Frame{}: <image>'.format(i + 1) for i in range(len(num_patches_list))])
        ...
        #     pixel_value = [transform(tile) for tile in img]
        #     pixel_values = torch.stack(pixel_value)
    ...
    def load_video(self, video_path, bound=None, input_size=448, max_num=1, num_segments=8):
        ...
        for frame_index in frame_indices:
            ...
            ret, frame = cap.read()
            if not ret:
                print(f"Warning: unable to retrieve frame {frame_index}")
                continue
            
            # 转换为 RGB 格式并创建 PIL Image 对象
