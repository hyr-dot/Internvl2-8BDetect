...
def load_ip_config():
    import yaml
    import os  
    yaml_path = "./ipconfig.yaml"
    if not os.path.exists(yaml_path):
        print('ip_config file is not exists:', yaml_path)
        exit(0)  
    with open(yaml_path) as f:
        ip_config = yaml.load(f.read(), Loader=yaml.FullLoader)
    return ip_config

ip_config = load_ip_config()
...
def video_test():
    ...
    try:
        ...
        if params:
            ...
            print('receive:', params_keys)
            filepath = params.get('filepath', None)
            if type(filepath) != str:
                return json.dumps({'state': 0,
                                'info': f'PARAMETER_TYPE_ERROR: expect {filepath} to be str, get {type(filepath)} instead!'},
                                ensure_ascii=False)
        # pixel_values, num_patches_list = load_video(video_path, num_segments=32, max_num=1)
            if filepath.endswith('.mp4'):
                ...
                return json.dumps({'state': 0,
                                'info': f'PARAMETER_TYPE_ERROR: expect {filepath} to be mp4, jpg or png!'},
                                ensure_ascii=False)
            ...
            return json.dumps({'state': 200, 'text': response_data}, indent=4, ensure_ascii=False)
    except Exception as e:
    # Handle exceptions during video loading
        return json.dumps({'state': 500, 'info': 'An error occurred: ' + str(e)})


def run_flask():
    print('start ...')
    app.run(host='0.0.0', port=ip_config['video_test1']['port'], debug=False)
    print('end...')
